...
# Константы для Крымского моста
CRIMEAN_BRIDGE_LENGTH = 19  # км
CRIMEAN_BRIDGE_START = (45.3005, 36.5125)  # координаты начала моста (материковая сторона)
CRIMEAN_BRIDGE_END = (45.2779, 36.5611)    # координаты конца моста (крымская сторона)

# Границы Крыма для определения, находится ли точка в Крыму
CRIMEA_BOUNDS = {
    'min_lat': 44.0,
    'max_lat': 46.5,
    'min_lon': 32.0,
...
def calculate_route_with_crimea(start_coords, all_coords):
    """
    Рассчитывает маршрут с учетом Крымского моста.
    Если маршрут проходит через Крым, разбивает его на две части.
    """
    ...
    for coord in all_coords:
        if coord and is_in_crimea(coord[0], coord[1]):
            crimea_points.append(coord)
        else:
            other_points.append(coord)
    
    # Если нет точек в Крыму, считаем обычный маршрут
    if not crimea_points:
        full_coordinates = [start_coords] + all_coords
        return ors_route_with_waypoints(full_coordinates)
    
    # Если есть точки и в Крыму, и вне Крыма, разбиваем маршрут
    if crimea_points and other_points:
        # 1. Маршрут от старта до последней точки вне Крыма
        non_crimea_route = []
        current_section = [start_coords]
        
        for coord in all_coords:
            if coord in crimea_points:
                break  # Дошли до первой точки в Крыму
            current_section.append(coord)
        
        if len(current_section) > 1:
            dist_non_crimea = ors_route_with_waypoints(current_section)
            if dist_non_crimea:
                non_crimea_route.append(dist_non_crimea)
        
        # 2. Добавляем расстояние до Крымского моста
        # (предполагаем, что от последней точки вне Крыма до моста - 0 км, так как мост уже в расчете)
        # Вместо этого, добавим маршрут от последней точки до начала моста
        if current_section:
            bridge_start_section = [current_section[-1], CRIMEAN_BRIDGE_START]
            dist_to_bridge = ors_route_with_waypoints(bridge_start_section)
            if dist_to_bridge:
                non_crimea_route.append(dist_to_bridge)
        
        # 3. Добавляем длину моста
        non_crimea_route.append(CRIMEAN_BRIDGE_LENGTH)
        
        # 4. Маршрут от конца моста до точек в Крыму
        crimea_section = [CRIMEAN_BRIDGE_END] + crimea_points
        dist_crimea = ors_route_with_waypoints(crimea_section)
        
        if dist_crimea:
            # Суммируем все расстояния
            total_distance = sum([d for d in non_crimea_route if d]) + dist_crimea
            return total_distance
        else:
            return None
    
    # Если все точки в Крыму
    elif crimea_points and not other_points:
        # Маршрут от старта до начала моста
        to_bridge = [start_coords, CRIMEAN_BRIDGE_START]
        dist_to_bridge = ors_route_with_waypoints(to_bridge)
        
        if dist_to_bridge:
            # Маршрут от конца моста до точек в Крыму
            from_bridge = [CRIMEAN_BRIDGE_END] + crimea_points
            dist_from_bridge = ors_route_with_waypoints(from_bridge)
            
            if dist_from_bridge:
                total_distance = dist_to_bridge + CRIMEAN_BRIDGE_LENGTH + dist_from_bridge
                return total_distance
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🌉 Особенность: автоматический учет Крымского моста (19 км) при маршрутах в Крым."
    )
    ...
    for route in routes:
        try:
            ...
            if geocode_errors or not start_coords or not all_coords:
                ...
                # Проверяем, есть ли точки в Крыму
                has_crimea = any(is_in_crimea(coord[0], coord[1]) for coord in all_coords)
                
                if has_crimea:
                    # Используем специальную функцию для маршрутов через Крым
                    distance = calculate_route_with_crimea(start_coords, all_coords)
                    time.sleep(1)  # Задержка между запросами
                else:
                    # Строим обычный маршрут: стартовая точка + все точки из цепочки
                    full_coordinates = [start_coords] + all_coords
                ...
                if distance:
                    ...
                    ws.cell(row=row_num, column=3).value = "✅ Успешно" + (" (с учетом Крымского моста)" if has_crimea else "")
...
• Разделение маршрута на две части: до моста и после моста
...
async def example_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "| Ростов-на-Дону, Оганова 22 | р. Крым, г. Симферополь, ул. Куйбышева 251 |\n\n"
